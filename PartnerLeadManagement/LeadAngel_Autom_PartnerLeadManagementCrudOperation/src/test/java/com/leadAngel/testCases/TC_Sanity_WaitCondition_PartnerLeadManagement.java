...
@Test
public class TC_Sanity_WaitCondition_PartnerLeadManagement extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_WaitCondition_PartnerLeadManagement.class);
	public void PartnerLeadManagementTest() throws InterruptedException {
		...
		try {
			LMP.ClickOnWaitAttr();
			Logger.info("Clicked on Wait Button is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Clicked on Wait Button is FAILED!!", e);
			sa.assertAll();
		}
