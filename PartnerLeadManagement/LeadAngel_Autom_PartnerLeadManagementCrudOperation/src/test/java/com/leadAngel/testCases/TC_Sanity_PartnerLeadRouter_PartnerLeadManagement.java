...
@Test
public class TC_Sanity_PartnerLeadRouter_PartnerLeadManagement extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_PartnerLeadRouter_PartnerLeadManagement.class);
	...
	public void PartnerLeadManagementTest() throws InterruptedException {
		...
		for (int i = 1; i < CountAllLinks; i++) {//CountAllLinks
			...
			if (getLinkTextAssignmentFirstListBox.contains("Assign Based on a Lookup Field in the Parent Account")==true ||
				...
				if (getLinkTextAssignmentFirstListBox.contains("Assign Based on a Lookup Field in the Parent Account")==true) {
					...
					try {
						if (LPWR.DisplayedAssignOnlyToUsersIfAvailableOnIntegratedCalendarCheckBoxIsChecked()==true) {
				...
				if (getLinkTextAssignmentFirstListBox.contains("Assign to Parent Accounts Owner")==true) {
					...
					try {
						...
						Logger.info("Click on CappingCondition DropDown is Passed !!");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Click on CappingCondition DropDown is Failed !!", e);
						sa.assertAll();
					}
					Thread.sleep(1500);
					try {
			...
			else if (getLinkTextAssignmentFirstListBox.contentEquals("Assign to Partner Rep")==true ||
					getLinkTextAssignmentFirstListBox.contentEquals("Assign to Partner Team")==true) {
				Thread.sleep(1500);
				try {
					LPWR.SelectDropDownForPartnerTeam();
					Logger.info("Click on main second Select DropDown is Passed");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Click on main second Select DropDown is Failed", e);
				...
				if (getLinkTextAssignmentFirstListBox.contentEquals("Assign to Partner Team")==true) {
					Thread.sleep(1500);
					try {
						LPWR.SelectOptionForPartnerTeam();
						Logger.info("Select PartnerTeam Option is Passed !!");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Select PartnerTeam Option is Failed !!", e);
						sa.assertAll();
					} 
				}
					...
					Thread.sleep(1500);
					try {
						LPWR.SelectOptionForPartnerRep();
						Logger.info("Select PartnerRep Option is Passed !!");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Select PartnerRep Option is Failed !!", e);
						sa.assertAll();
					} 
				} 
			}
		...
		try {
			...
			Logger.info("Default LeadOwner Option is Passed!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Default LeadOwner Option is Failed!!", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
		...
		for (int D = 0; D < 4; D++) {//D
			Thread.sleep(2500);
			try {
				LMP.DataValueUpdateWhenRoutedToDefaultLeadOwnerDropDownForPartner();
			...
			if (D==0) {//j0
				...
				for (int k = 0; k <4; k++) {//k4
					...
					if (k==0) {
						...
						Thread.sleep(2500);
						try {
							LR.CheckBoxForDataValueUpdateWhenRoutedToDefaultLeadOwner();
				...
				try {
					...
					sa.assertTrue(false);
					Logger.error("Click On ConvertedDate Calendar Button Box1 is FAILED!!!", e);
					sa.assertAll();
				}
				Thread.sleep(2500);
				try {
				...
				Thread.sleep(2000);
				try {
					LR.DataValueUpdateWhenRoutedToDefaultLeadOwnerDropDown1();
					Logger.info("Click on DataValueUpdateWhenRoutedToDefaultLeadOwner DropDown1 is Passed!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Click on DataValueUpdateWhenRoutedToDefaultLeadOwner DropDown1 is Failed!!", e);
					sa.assertAll();
				}
				Thread.sleep(2500);
				try {
		...
		Thread.sleep(3500);
		try {
			LR.ClickOnPartnerLeadRouterFeature();
			Logger.info("Click On Partner Lead Router Feature is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on Partner Lead Router Feature is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
			if(PLM.PartnerLeadRouterOverview()==true) {
				Logger.info("Display PartnerLeadRouter OverView Page is Passed");
			}
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Display PartnerLeadRouter OverView Page is Failed", e);
			sa.assertAll();
		}
		...
		try {
			...
			Logger.info("Click On DeleteCraft button is PASSED!!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Click On DeleteCraft button is FAILED!", e);
			sa.assertAll();
		}
		Thread.sleep(5500);
		Thread.sleep(7000);
