...
@Test
public class TC_CrudOperation_WaitCondition_PartnerLeadManagement extends BaseClass {
	private static final Logger Logger = LogManager.getLogger(TC_CrudOperation_WaitCondition_PartnerLeadManagement.class);
	public void PartnerLeadManagementTest() throws InterruptedException {
		...
		try {
			LMP.ClickOnNewObject();
			Logger.info("Clicked on New PartnerLeadRouter is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Clicked on New PartnerLeadRouter is Failed",e);
			sa.assertAll();
		}
