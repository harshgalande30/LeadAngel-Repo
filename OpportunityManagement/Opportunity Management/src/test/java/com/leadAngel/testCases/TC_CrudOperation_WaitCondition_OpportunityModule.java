...
@Test
public class TC_CrudOperation_WaitCondition_OpportunityModule extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_CrudOperation_WaitCondition_OpportunityModule.class);
	...
	public void OpportunityManagement() throws InterruptedException {
		...
		try {
			LMP.InputWaitFor();
			Logger.info("Give an Input to WaitFor is PASSED!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Give an Input to WaitFor is FAILED!!!", e);
			sa.assertAll();
		}
		Thread.sleep(5200);
		try {
