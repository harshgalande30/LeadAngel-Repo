...
public class AccountMaanagement_PageObjectModel {
	...
	//************************************************** Account Routing Territory ********************************************
		...
		@FindBy(xpath = "//i[@class='righticon-STRING']//..//span[text()='Opportunity Id']")
		WebElement Opportunity_DND;
		...
		public void Opportunity_DND() {
			new WebDriverWait(ldriver, Duration.ofSeconds(30)).until(ExpectedConditions.elementToBeClickable(Opportunity_DND));
			Actions actions = new Actions(ldriver);
			int x = AddField.getLocation().x;
			int y = AddField.getLocation().y;
			actions.moveToElement(Opportunity_DND).pause(Duration.ofSeconds(0)).clickAndHold(Opportunity_DND)
			.pause(Duration.ofSeconds(0))
			.moveToElement(AddField).moveByOffset(x, y).pause(Duration.ofSeconds(0)).release().build().perform();
		}
