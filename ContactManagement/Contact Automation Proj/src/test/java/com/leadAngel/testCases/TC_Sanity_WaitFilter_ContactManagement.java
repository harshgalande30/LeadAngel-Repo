...
@Test
public class TC_Sanity_WaitFilter_ContactManagement extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_WaitFilter_ContactManagement.class);
	...
	public void ContactManagement() throws InterruptedException {
		...
		if(ae.DisplaySameAEerror() == true) {
			...
			try {
				...
				Logger.info("Clcik on Wait Filter DropDown is PASSED!!");
			} catch (Exception e) {
				sa.assertTrue(false);
		...
		for (int i=1;i<countAboutMe;i++) {
			...
			if (getLinkTextFirstName.contains("contains") || getLinkTextFirstName.contains("does not contain")
				...
				try {
					...
					Logger.info("Enter free text OtherLongitude is PASSED!!");
				} catch (Exception e) {
					rrp.TextAreaLongitudeAlphabet();
					Logger.error("Enter free text OtherLongitude is Failed!!",e);
				}
				Thread.sleep(1500);
				try {
					ALP.ClickOnSavebtn();
		...
		for (int M = 1; M <countCreatedDate; M++) {
			...
			if (getLinkTextCreatedDate.contentEquals("before")==true||getLinkTextCreatedDate.contentEquals("after")==true||getLinkTextCreatedDate.contentEquals("on or before")==true
				...
				try {
					...
					Logger.info("Clicked On CreatedDate Calendar Button Box2 is PASSED!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Clicked On CreatedDate Calendar Button Box2 is FAILED!!!", e);
					sa.assertAll();
				}
				Thread.sleep(1500);
				try {
