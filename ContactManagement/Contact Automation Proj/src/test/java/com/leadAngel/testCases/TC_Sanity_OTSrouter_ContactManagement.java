...
@Test
public class TC_Sanity_OTSrouter_ContactManagement extends BaseClass {
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_OTSrouter_ContactManagement.class);
	...
	public void ContactManagement() throws InterruptedException {
		...
		Thread.sleep(2500);
		try {
			LR.ClickOnSecondSelectBoxDropdownActionBlockOTSCR();
		...
		for (int i = 0; i < CountAllLinks; i++) {//countAllLinksof Assignment type
			...
			if (getLinkTextAssignmentTypeListBox.contains("Live Claim")==true ||
				...
				for (int j = 0; j < CountAllLinksAssignmentOption; j++) {//AssignmentOption
					...
					if (getLinkTextAssignmentOptionListBox.contains("Assign Based on a Lookup Field in the Parent Account")==true ) {
						...
						try {
							...
							getLinkTextAssignmentOptionListBox.contains("Assign to Sales Rep Email Lookup on Contact")==true ||
							getLinkTextAssignmentOptionListBox.contains("Assign to Sales Rep ID Lookup on Contact")==true ||
					...
					try {
						...
						Logger.info("Clicked On Save Button For OTSAssignmentBlock is Passed");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Clicked On Save Button For OTSAssignmentBlock is Failed", e);
						sa.assertAll();
					}
					Thread.sleep(2000);
					try {
				...
				if (getLinkTextAssignmentTypeListBox.contains("Live Claim")==true) {//Claim
					Thread.sleep(4000);
					try {
						LR.DoubleClickOnOTSAssignmentBlock();
						Logger.info("Double Click On OTS Assignment Block is Passed !!");	
					...
					Thread.sleep(3500);
					try {
						OTS.ClickOnAcceptLeadDropDown();
						Logger.info("Click On AcceptLead DropDown is Passed");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Click On AcceptLead DropDown is Failed",e);
						sa.assertAll();
					}
					Thread.sleep(2500);
					try {
						OTS.SelectOnAcceptLeadInAnyoneWithTheLinkCanAcceptOption();
						Logger.info("Select AcceptLead In AnyoneWithTheLinkCanAccept Option is Passed");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Select AcceptLead In AnyoneWithTheLinkCanAccept Option is Failed",e);
						sa.assertAll();
					}
					Thread.sleep(2000);
					try {
						OTS.ClickOnRejectLeadDropDown();
						Logger.info("Click On RejectLead DropDown is Passed");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Click On RejectLead DropDown is Failed",e);
						sa.assertAll();
					}
					Thread.sleep(2000);
					try {
						OTS.SelectMemberOfSalesTeamOption();
						Logger.info("Select MemberOfSalesTeam Option is Passed");
					} catch (Exception e) {
						sa.assertTrue(false);
						Logger.error("Select MemberOfSalesTeam Option is Failed",e);
						sa.assertAll();
					}
					Thread.sleep(2500);
					try {
					...
					List<WebElement> AllLinksOfSalesTeamTypeListBox1 = driver.findElements(By.xpath("//div[@role='listbox']//mat-option//span[text()]"));
					...
					for (int k = 0; k < CountAllLinksSalesTeamType; k++) {
						...
						if (getLinkTextSalesTeamTypeListBox.contains("Round Robin Team")==true||
								getLinkTextSalesTeamTypeListBox.contains("Weighted Sales Team")==true) {
						...
						try {
							LR.ClickedOnSaveButtonForAssignmentBlock();
							Logger.info("Click On Save Button For OTSAssignmentBlock is Passed");
						} catch (Exception e) {
							sa.assertTrue(false);
							Logger.error("Click On Save Button For OTSAssignmentBlock is Failed", e);
							sa.assertAll();
						}
						Thread.sleep(2000);
						try {
			...
			else if (getLinkTextAssignmentTypeListBox.contains("Broadcast")==true) {//BroadCast
		...
		for (int i = 0; i < CountAllLinks2; i++) {//countAllLinksof Assignment type
			...
			if (getLinkTextAssignmentTypeListBox.contains("Live Claim")==true ||
				...
				Thread.sleep(2000);
				try {
					OTS.ClickSelectEmailTemplateDropDownAssignmentSetupCR();
				...
				Thread.sleep(2000);
				try {
					OTS.ClickSelectTextTemplateDropDownAssignmentSetupCR();
				...
				try {
					...
					Logger.info("Click on CollapseAll For OTS assignment is Passed");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Click on CollapseAll For OTS assignment is Failed", e);
					sa.assertAll();
				}	
				Thread.sleep(2000);
				try {
		...
		try {
			...
			Logger.info("Default ContactOwner DropDown is Passed!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Default ContactOwner DropDown is Failed!!", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
			detrrt.SelectDefaultLeadOwnerLPWROption();
			Logger.info("Select Default ContactOwner Option is Passed!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Select Default ContactOwner Option is Failed!!", e);
			sa.assertAll();
		}
