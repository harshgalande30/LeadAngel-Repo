...
public class TC_Sanity_AcceptanceConfirmationCRM_Administration extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_AcceptanceConfirmationCRM_Administration.class);

	@Test(description = "Administration_AcceptanceConfirmationCRM_Section")
	@Step("Administration_AcceptanceConfirmationCRM_Section")
	@Description("Administration_AcceptanceConfirmationCRM_Section")
	public void AdministrationTest() throws InterruptedException, AWTException {
		...
		Thread.sleep(2500);
		try {
			OTSnt.ClickOnAcceptanceConfirmationCRM();
			Logger.info("Click On AcceptanceConfirmationCRM is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Click On AcceptanceConfirmationCRM is Failed",e);
			sa.assertAll();
		}
		...
		for (int k = 0; k < 2; k++) {
			...
			if (k==1) {
				Logger.info("========================================= Contact AcceptanceConfirmationCRM Operation ======================================");

				WebElement ContactOTSScreenAssignmentConfirmation = driver.findElement(By.xpath("//li[text()=' Contact : Acceptance Confirmation - CRM ']"));
				Actions action=new Actions(driver);
				action.moveToElement(ContactOTSScreenAssignmentConfirmation).click().perform();
				Logger.info("Move the mouse cursor to Contact Acceptance Confirmation CRM is PASSED!!");

				Thread.sleep(3000);
				try {
					OTSnt.ClickOnContactAcceptanceConfirmationCRMTab();
					Logger.info("Click On Contact AcceptanceConfirmationCRM Tab is Passed");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Click On Contact AcceptanceConfirmationCRM Tab is Failed",e);
					sa.assertAll();
				}
				Thread.sleep(1500);
				try {
				...
				try {
					OTSnt.ClickOnRestoreDefaultButton();
					Logger.info("Click On RestoreDefault Button for Contact AcceptanceConfirmationCRM is Passed");
				}catch(Exception e) {
					sa.assertTrue(false);
					Logger.error("Click On RestoreDefault Button for Contact AcceptanceConfirmationCRM is Faileed",e);
					sa.assertAll();
				}
				...
				try {
					if (OTSnt.locationOfName.contentEquals(locationOfNameContact)&&OTSnt.NameTextContact.contentEquals(NameTextContact)) {
						Logger.info(NameTextContact+" is perfectly located in Contact AcceptanceConfirmationCRM Message Body");
					}
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error(NameTextContact+" is not perfectly located in Contact AcceptanceConfirmationCRM Message Body",e);
					sa.assertAll();
				}
				Thread.sleep(2500);
				...
				try {
					if (OTSnt.locationOfEmail.contentEquals(locationOfEmailContact)&&OTSnt.EmailTextContact.contentEquals(EmailTextContact)) {
						Logger.info(EmailTextContact+" is perfectly located in Contact AcceptanceConfirmationCRM Message Body");
					}
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error(EmailTextContact+" is not perfectly located in Contact AcceptanceConfirmationCRM Message Body",e);
					sa.assertAll();
				}
			}
			...
			for (int i = 0; i < CountAllLinks; i++) {
				...
				if (i!=0) {
					...
					try {
						APOM.ClickOnInsertSnippetButtonForEmailBody();
						Logger.info("Click On Insert Snippe tButton For Message Body is PASSED!!");
					}catch(Exception e) {
						sa.assertTrue(false);
						Logger.error("Click On Insert Snippe tButton For Message Body is FAILED!!!",e);
						sa.assertAll();
					}
					Thread.sleep(2500);
					try {
						EATP.ClickOnSelectObjectDropDownForInsertValueInSubject();
						Logger.info("Click On Select Object DropDown ForInsert Value is PASSED!!");
					}catch(Exception e) {
						sa.assertTrue(false);
						Logger.error("Click On Select Object DropDown For Insert Value is FAILED!!!",e);
						sa.assertAll();
			...
			try {
				...
				Logger.info("Enter Text Inside Message body is PASSED!!");
			}catch(Exception e) {
				sa.assertTrue(false);
				Logger.error("Enter Text Inside Message body is FAILED!!!",e);
				sa.assertAll();
			}
			Thread.sleep(2500);
			Robot r=new Robot();
			...
			Logger.info("Selected all the text from the AcceptanceConfirmationCRM message body ");
			...
			if (k==1) {Thread.sleep(2500);
			WebElement NameContentContact = driver.findElement(By.xpath("//font[text()='Name : {!ContactOwner.Name}']"));
			String NameTextContact = NameContentContact.getText();
			String locationOfNameContact=NameContentContact.getLocation().toString();
			try {
				if (OTSnt.locationOfName.contentEquals(locationOfNameContact)&&OTSnt.NameTextContact.contentEquals(NameTextContact)) {
					Logger.info(NameTextContact+" is perfectly located in Contact Acceptance Confirmation CRM Message Body");
				}
			} catch (Exception e) {
				sa.assertTrue(false);
				Logger.error(NameTextContact+" is not perfectly located in Contact Acceptance Confirmation CRM Message Body",e);
				sa.assertAll();
			}
			Thread.sleep(2500);
			WebElement EmailContentContact = driver.findElement(By.xpath("//font[text()='Email: {!ContactOwner.Email}']"));
			...
			try {
				if (OTSnt.locationOfEmail.contentEquals(locationOfEmailContact)&&OTSnt.EmailTextContact.contentEquals(EmailTextContact)) {
					Logger.info(EmailTextContact+" is perfectly located in Contact Acceptance Confirmation CRM Message Body");
				}
			} catch (Exception e) {
				sa.assertTrue(false);
				Logger.error(EmailTextContact+" is not perfectly located in Contact Acceptance Confirmation CRM Message Body",e);
				sa.assertAll();
			}}
			...
			try {
				...
				Logger.info("Displayed hasPopup Acceptance Confirmation CRM is Passed");
			}catch(Exception e) {
				sa.assertTrue(false);
				Logger.error("Displayed hasPopup Acceptance Confirmation CRM is Failed",e);
				sa.assertAll();
			}
			continue;
		}//End loop
