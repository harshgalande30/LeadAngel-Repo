...
public class TC_Sanity_FailedOTSinitiation_Administration extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_FailedOTSinitiation_Administration.class);

	@Test(description = "Administration_FailedOTSinitiation_Section")
	@Step("Administration_FailedOTSinitiation_Section")
	@Description("Administration_FailedOTSinitiation_Section")
	public void AdministrationTest() throws InterruptedException, AWTException {
		...
		Thread.sleep(2500);
		try {
			OTSnt.ClickOnFailedOTSInitiation();
			Logger.info("Click On FailedOTSInitiation is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Click On FailedOTSInitiation is Failed",e);
			sa.assertAll();
		}

		Logger.info("=========================================  LeadFailedOTSInitiationTab Operation ======================================");
		Thread.sleep(2500);
		try {
		...
		Thread.sleep(1000);
		try {
			driver.findElement(By.xpath("//div[@style='text-align: center;']//font[text()='CRM validation failed!']")).isDisplayed();
			Logger.info("'CRM validation failed!' is perfrctly located in center of the Message Body is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.info("'CRM validation failed!' is perfrctly located in center of the Message Body is failed",e);
			sa.assertAll();
		}
		Thread.sleep(1000);
		try {
			driver.findElement(By.xpath("(//div[@style='text-align: center;'])[2]")).isDisplayed();
			Logger.info("'The <<Object Type>>  information does not align with our validation criteria' is perfrctly located in center of the Message Body is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.info("'The <<Object Type>>  information does not align with our validation criteria' is perfrctly located in center of the Message Body is failed",e);
			sa.assertAll();
		}
		for (int k = 0; k < 2; k++) {

			if (k==1) {
				Logger.info("=========================================  ContactFailedOTSInitiationTab Operation ======================================");

				WebElement ContactFailedOTSInitiation = driver.findElement(By.xpath("//li[text()=' Contact : Failed OTS Initiation ']"));
				Actions action=new Actions(driver);
				action.moveToElement(ContactFailedOTSInitiation).click().perform();
				Logger.info("Move the mouse cursor to ContactFailedOTSInitiation is PASSED!!");

				Thread.sleep(3000);
				try {
					OTSnt.ClickOnContactFailedOTSInitiationTab();
					Logger.info("Click On Contact FailedOTSInitiation Tab is Passed");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Click On Contact FailedOTSInitiation Tab is Failed",e);
					sa.assertAll();
				}
				Thread.sleep(1500);
				try {
				...
				try {
					OTSnt.ClickOnRestoreDefaultButton();
					Logger.info("Click On RestoreDefault Button for Contact FailedOTSinitiation is Passed");
				}catch(Exception e) {
					sa.assertTrue(false);
					Logger.error("Click On RestoreDefault Button for Contact FailedOTSinitiation is Faileed",e);
					sa.assertAll();
				}
			...
			try {
				...
				Logger.info("Enter Text Inside FailedOTSinitiation Message body is PASSED!!");
			}catch(Exception e) {
				sa.assertTrue(false);
				Logger.error("Enter Text Inside FailedOTSinitiation Message body is FAILED!!!",e);
				sa.assertAll();
			}
			Thread.sleep(2500);
			Robot r=new Robot();
			...
			Logger.info("Selected all the text from the FailedOTSinitiation Message body");
			...
			Thread.sleep(2500);
			try {
				OTSnt.DisplayedhasPopupFailedOTSInitiation();
				Logger.info("Displayed hasPopup FailedOTSInitiation is Passed");
			}catch(Exception e) {
				sa.assertTrue(false);
				Logger.error("Displayed hasPopup FailedOTSInitiation is Failed",e);
				sa.assertAll();
			}
			continue;
		}//End loop
