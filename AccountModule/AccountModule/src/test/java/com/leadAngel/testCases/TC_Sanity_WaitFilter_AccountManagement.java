...
public class TC_Sanity_WaitFilter_AccountManagement extends BaseClass{
	private static final Logger Logger = LogManager.getLogger(TC_Sanity_WaitFilter_AccountManagement.class);
	@Test(description = "AccountManagement_WaitFilter _Section")
	@Step("AccountManagement_WaitFilter")
	...
	public void AccountManagementTest() throws InterruptedException {
		...
		try {
			...
			Logger.info("Click on AccountAttribute is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on AccountAttribute is FAILED!!",e);
			sa.assertAll();
		}

		Logger.info("//===================================== DRAG&DROP AccountName ATTRIBUTE =====================================//");
		Thread.sleep(4000);
		try {
			LF.AccountName_DND();
			Logger.info("Drag And Drop Account Name is PASSED!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Drag And Drop Account Name is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(3500);
		try {
			LF.ClickDropDownAccountName();
			Logger.info("Click on DropDown AccountName is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on DropDown AccountName is Failed", e);
			sa.assertAll();
		}
		List<WebElement> AllLinksOfFirstName = driver.findElements(By.xpath("//label[text()='Account Name']/..//select//option[text()]"));
		...
		for (int i=1;i<countAboutMe;i++) {
			...
			Thread.sleep(500);
			try {
				if( LR.DisplayedAccountNameSelectBoxNgValid()==true) {
			...
			if (getLinkTextFirstName.contains("contains") || getLinkTextFirstName.contains("does not contain")
				...
				Thread.sleep(1500);
				try {
					LF.AccountNameContainsInputforSanity();
					Logger.info("Input on AcountName is PASSED!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Input on AcountName is FAILED!!", e);
					sa.assertAll();
				}	
				Thread.sleep(500);
				try {
					if( LR.DisplayedAccountNameInputBoxNgValid()==true) {
		...
		Logger.info("//======================================= DRAG&DROP Billing LONGITUDE ATTRIBUTE ==================================//");
		Thread.sleep(4000);
		try {
			LF.BillingLongitude_DND();
			Logger.info("Drag and Drop Billing Longitude is PASSED!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Drag and Drop Billing Longitude is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
			LF.ClickDropDownBillingLongitude();
			Thread.sleep(2500);
			LF.ClickDropDownBillingLongitude();
			Logger.info("Click on DropDown Billing Longitude is Passed");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on DropDown Billing Longitude is Failed", e);
			sa.assertAll();
		}
		List<WebElement> AllLinksOfLongitude = driver.findElements(By.xpath("//label[text()='Billing Longitude ']/..//select//option[text()]"));
		...
		for (int j =1; j < countLongitude; j++) {
			...
			Thread.sleep(500);
			try {
				if( LR.DisplayedBillingLongitudeSelectBoxNgValid()==true) {
					Logger.info("Displayed Select Box is not red");
				}
			} catch (Exception e) {
				sa.assertTrue(false);
			...
			if (getLinkTextLongitude.contains("is empty")||getLinkTextLongitude.contains("is not empty")) {
				...
				Thread.sleep(500);
				try {
					if( LR.DisplayedBillingLongitudeInputBoxNgValid1()==true) {
				...
				Thread.sleep(1500);
				try {
					LF.BillingLongitudeInputBoxForIsGreaterThan();
					sa.assertTrue(true);
					Logger.info("Input on Billing Longitude is PASSED!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Input on Billing Longitude is FAILED!!", e);
					sa.assertAll();
				}
				Thread.sleep(500);
				try {
				...
				Thread.sleep(1500);
				try {
					LF.BillingLongitudeInputBoxForBetweenBox1();
					sa.assertTrue(true);
					Logger.info("Billing Longitude Input Box For Between Box1 is PASSED!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Billing Longitude Input Box For Between Box1 is FAILED!!", e);
					sa.assertAll();
				}
				Thread.sleep(500);
				try {
				...
				Thread.sleep(1500);
				try {
					LF.BillingLongitudeInputBoxForBetweenBox2();
					sa.assertTrue(true);
					Logger.info("Billing Longitude Input Box For Between Box2 is PASSED!!");
				} catch (Exception e) {
					sa.assertTrue(false);
					Logger.error("Billing Longitude Input Box For Between Box2 is FAILED!!", e);
					sa.assertAll();
				}
				Thread.sleep(500);
				try {
					if( LR.DisplayedBillingLongitudeInputBoxNgValid2()==true) {
