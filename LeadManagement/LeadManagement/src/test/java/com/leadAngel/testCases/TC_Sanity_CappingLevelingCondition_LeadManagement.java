...
public class TC_Sanity_CappingLevelingCondition_LeadManagement extends BaseClass{
	private final static Logger Logger = org.apache.logging.log4j.LogManager
			.getLogger(TC_Sanity_CappingLevelingCondition_LeadManagement.class);
	...
	public void LeadManagementTest() throws InterruptedException {
		...
		try {
			...
			Logger.info("Click on New CappingLeveling is PASSED!!");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on New CappingLeveling is FAILED", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
		...
		if(ae.DisplaySameAEerror() == true) {
			...
			try {
				...
				Logger.info("Click On Capping levelling DropDown is PASSED!!");
			} catch (Exception e) {
				sa.assertTrue(false);
				Logger.error("Click On Capping levelling DropDown is FAILED!", e);
				sa.assertAll();
			}
			Thread.sleep(2500);
			try {
		...
		try {
			...
			if( LMP.DisplayCappingLevelingConditionOverview()==true) {
				Logger.info("Display CappingLevelingCondition Overview passed");
				Thread.sleep(2500);
				try {
					LMP.ClickOnCapping_levellingDropDown();
					Logger.info("Click On Capping levelling DropDown is PASSED!!");
		...
		Thread.sleep(3500);
		try {
			LR.ClickOnCappingLevelingConditionFeature();
			Logger.info("Click On CappingLevelingCondition Feature is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on CappingLevelingCondition Feature is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(5000);
		try {
			if( LMP.DisplayCappingLevelingConditionOverview()==true) {
				Logger.info("Display CappingLevelingCondition OverView Page is Passed");
			}
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Display CappingLevelingCondition OverView Page is Failed", e);
			sa.assertAll();
		}
