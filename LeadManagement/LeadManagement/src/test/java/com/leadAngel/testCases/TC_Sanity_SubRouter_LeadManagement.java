...
public class TC_Sanity_SubRouter_LeadManagement extends BaseClass {
	private final static Logger Logger = org.apache.logging.log4j.LogManager
			.getLogger(TC_Sanity_SubRouter_LeadManagement.class);	
	...
	public void LeadManagementTest() throws InterruptedException {
		...
		try {
			...
			sa.assertTrue(false);
			Logger.info("Enter AutoTestObject name is FAILED!!!",e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
		...
		if(ae.DisplaySameAEerror() == true) {
			...
			try {
				...
				Logger.info("Click on SubRouter DropDown is Passed");
			}catch(Exception e) {
				sa.assertTrue(false);
				Logger.error("Click on SubRouter DropDown is FAILED!!", e);
				sa.assertAll();
			}
			Thread.sleep(2500);
			try {
			...
			try {
				...
				Logger.info("Click agian Delete Button is Passed!!!");
			} catch (Exception e) {
				sa.assertTrue(false);
				Logger.error("Click agian Delete Button is FAILED!", e);
				sa.assertAll();
			}
			Thread.sleep(400);
			try {
		...
		try {
			...
			if(LMP.DisplaySubRouterOverview()==true) {
				Logger.info("Displayed SubRouter Overview ");
				{
					Thread.sleep(2600);
					try {
						LMP.DropDownSubRouter();
		...
		for (int i = 1; i < CountAllLinks; i++) {//CountAllLinks
			...
			if (getLinkTextAssignmentFirstListBox.contains("Assign Based on a Lookup Field in the Parent Account")==true ||
				...
				if (getLinkTextAssignmentFirstListBox.contains("Assign to Parent Accounts Owner")==true) {
					...
					Thread.sleep(2000);
					try {
						LR.ClickIfNoOneIsAvailableHoldTheLeadCheckBox();
		...
		try {
			...
			Logger.info("Check Box For DefaultLeadOwner is Passed!!");
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Check Box For DefaultLeadOwner is Failed!!", e);
			sa.assertAll();
		}
		Thread.sleep(900);
		try {
				...
				for (int k = 0; k <4; k++) {//k4
					...
					if (k==0) {
						...
						try {
							...
							sa.assertTrue(false);
							Logger.error("Select SetToText For Data value update in Details page is Failed !!", e);
							sa.assertAll();
						}
						Thread.sleep(1500);
						try {
		...
		Thread.sleep(3500);
		try {
			LR.ClickOnSubRouterFeature();
			Logger.info("Click On Sub Router Feature is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on Sub Router Feature is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(2500);
		try {
			if(LMP.DisplaySubRouterOverview()==true) {
				Logger.info("Display SubRouter OverView Page is Passed");
			}
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Display SubRouter OverView Page is Failed", e);
			sa.assertAll();
		}
