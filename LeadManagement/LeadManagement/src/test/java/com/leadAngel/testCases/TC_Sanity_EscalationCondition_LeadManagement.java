...
public class TC_Sanity_EscalationCondition_LeadManagement extends BaseClass{
	private final static Logger Logger = org.apache.logging.log4j.LogManager
			.getLogger(TC_Sanity_EscalationCondition_LeadManagement.class);
	...
	public void LeadManagementTest() throws InterruptedException {
		...
		for (int j =1; j < countLongitude; j++) {
			...
			if (getLinkTextLongitude.contains("is empty")||getLinkTextLongitude.contains("is not empty")) {
				...
				try {
					...
					rrp.TextAreaLongitudeNumerical();
		...
		Thread.sleep(3500);
		try {
			LR.ClickOnEscalationConditionFeature();
			Logger.info("Click On EscalationCondition Feature is Passed");
		}catch(Exception e) {
			sa.assertTrue(false);
			Logger.error("Click on EscalationCondition Feature is FAILED!!", e);
			sa.assertAll();
		}
		Thread.sleep(5000);
		try {
			if( LMP.DisplayEscalationConditionOverview()==true) {
				Logger.info("Display EscalationCondition OverView Page is Passed");
			}
		} catch (Exception e) {
			sa.assertTrue(false);
			Logger.error("Display EscalationCondition OverView Page is Failed", e);
			sa.assertAll();
		}
