...
public class TC_Sanity_WaitFilter_LeadManagement extends BaseClass {
	private final static Logger Logger = org.apache.logging.log4j.LogManager.getLogger(TC_Sanity_WaitFilter_LeadManagement.class);
	@Test(description = "LeadManagement_LeadRouting _Section")
	@Step("LeadManagement_LeadRouting_WaitFilter")
	@Description("LeadManagement_LeadRouting_WaitFilter")
		...
		for (int i=1;i<countAboutMe;i++) {
			...
			if (getLinkTextFirstName.contains("contains") || getLinkTextFirstName.contains("does not contain")
				...
				try {
					...
					Logger.error("Type Into in TextArea is Failed!!");
				}	
				Thread.sleep(1000);
				try {
					ALP.ClickOnSavebtn();
		...
		for (int j =1; j < countLongitude; j++) {
			...
			if (getLinkTextLongitude.contains("is empty")||getLinkTextLongitude.contains("is not empty")) {
				...
				try {
					...
					Logger.info("Enter number in TextArea is PASSED!!");
				}		
				Thread.sleep(1500);
				try {
					ALP.ClickOnSavebtn();
